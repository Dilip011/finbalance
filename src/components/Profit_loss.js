...
const Profile_loss = () => {
  ...
  const fetchAndPopulateTable = useCallback(async (user_id) => {
    ...
    const collectionRef = collection(db, 'profit');
  ...
  async function addData() {
    ...
    try {
      const docRef = doc(collection(database, 'profit'));
  ...
  const handleUpdate = async () => {
    try {
      const q = query(collection(db, 'profit'), where('sno', '==', selectedSno));
      const querySnapshot = await getDocs(q);
  
      querySnapshot.forEach(async (document) => {
        const docRef = doc(db, 'profit', document.id);
  ...
  const handleDelete = async (snoToDelete) => {
    try {
      const q = query(collection(db, 'profit'), where('sno', '==', snoToDelete));
      const querySnapshot = await getDocs(q);
  ...
  const updateTable = async () => {
    try {
      const q = query(collection(db, 'profit'));
      const querySnapshot = await getDocs(q);
